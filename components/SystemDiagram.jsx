...
import { Activity, Globe, Repeat, Zap } from 'lucide-react';
			...
			<div className='relative z-10 mx-auto grid w-full max-w-7xl grid-cols-1 items-center gap-10 px-4 sm:px-6 lg:grid-cols-[1.1fr_1fr] lg:gap-20 lg:px-8'>
				...
				<motion.div
					...
					<p className='mb-4 max-w-[720px] text-base leading-8 text-white/75'>
						Standard solutions don’t create mature growth. No two businesses have the same
						constraints, customers, or goals.
					</p>

					<p className='max-w-[720px] text-base leading-8 text-white/75'>
						Synchroyst analyzes first. Then activates only what strengthens your engine. Nothing
						unnecessary runs.
					</p>
				</motion.div>
					...
					{items.map(item => {
						...
						return (
							<motion.div
								...
								<h3 className='mb-[10px] text-xl font-black text-white'>{item.title}</h3>

								<p className='max-w-[360px] text-sm leading-7 text-white/80'>{item.desc}</p>
							</motion.div>
						);
					})}
				</motion.div>
