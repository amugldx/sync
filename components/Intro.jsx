...
import { CheckCircle2, Cpu, Rocket, Zap } from 'lucide-react';
...
export default function Intro({ onFinish }) {
	...
	useEffect(() => {
		...
		const timer = setInterval(() => {
			const p = Math.min(100, Math.floor(((Date.now() - start) / 1500) * 100));
			setProgress(p);
			if (p >= 100) {
				clearInterval(timer);
				setPhase('ready');
				setTimeout(() => setPhase('message'), 1000);
						...
						<motion.div
							...
							<div className='relative'>
								...
								<div
									...
									style={{ width: `${progress}%` }}
								/>
								<span className='absolute -bottom-6 right-0 text-[10px] font-mono text-white/30'>
									{progress}%
								</span>
							</div>
						</motion.div>
