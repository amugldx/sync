...
export default function Modules() {
	const base = useMemo(
		() => [
			...
			{
				name: 'Customer Support (AI + Human)',
				on: false,
				desc: '24/7 hybrid response.',
				dir: 'left',
			},
			{ name: 'Lifecycle Follow-ups', on: true, desc: 'Nurturing automated loops.', dir: 'right' },
			{
				name: 'Retention + LTV Optimization',
				on: false,
				desc: 'Customer loyalty engine.',
				dir: 'top',
			},
	...
	return (
		<section
			className='relative overflow-hidden bg-[#05040a] py-32'
			id='Modules'>
				...
				<motion.div
					...
					{mods.map((m, idx) => (
						<motion.button
							...
							whileTap={{ scale: 0.98 }}
							className={`group relative flex flex-col items-start overflow-hidden rounded-[1rem] border p-8 text-left transition-all duration-500 ${
								m.on
							...
							<div
								className={`mb-6 rounded-2xl p-4 transition-all duration-500 ${
									m.on ? 'bg-blue-600 text-white' : 'bg-white/5 text-white/40'
								}`}>
								{getIcon(m.name)}
							</div>
								...
								<div className='mb-2 flex items-center justify-between'>
									<p
										className={`text-lg font-bold tracking-tight transition-colors duration-500 ${
											m.on ? 'text-white' : 'text-white/40'
										}`}>
										{m.name}
									</p>
									<div
										className={`h-1.5 w-1.5 rounded-full ${
											m.on ? 'animate-pulse bg-blue-400' : 'bg-white/10'
										}`}
									/>
								</div>
							...
							<div className='mt-auto flex items-center gap-3'>
								<div
									className={`relative h-5 w-10 rounded-full transition-all duration-500 ${
										m.on ? 'bg-blue-600' : 'bg-white/10'
									}`}>
									<motion.div
										animate={{ x: m.on ? 22 : 2 }}
								...
								<span
									className={`text-[10px] font-black tracking-widest transition-colors ${
										m.on ? 'text-blue-400' : 'text-white/20'
									}`}>
